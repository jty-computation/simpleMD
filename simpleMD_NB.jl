...
begin
	...
	Na			= 10;   		# number of positively charged particles
	Nb			= 10;	 		# number of negatively charged particles
	Δt			= 1;
	tf			= 100;
	""
end
...
function step!(force::Function, Pi::Particle,Ps::Vector{Particle})
	...
	# Update Acceleration
	Fij::Vector = zeros(length(Pi.pos)) 
...
# ╔═╡ 9f1d3f9f-245f-4a5d-8abe-193c81ba0d14
function simulate!(force::Function, plt, particles::Vector{Particle})
	[step!(force, Pi, particles) for Pi in particles]
	plt = scatter3d!([Tuple((P.pos)) for P in particles])
	return plt
end

# ╔═╡ cced9b76-a640-453f-a637-96a272c07494
# define magnitude
function mag(x)
	sqrt(x⋅x)
end
...
# ╔═╡ 7759d938-e73a-4150-8ea8-420aaac949db
function force(Pi::Particle,Pj::Particle)
	# returns Coulomb force between two particles
	# assumes k = 1 by default
	k			= 1
	displ 		= Pi.pos - Pj.pos              	# displacement::vector
	dist		= mag(displ)					# distance::scalar
	F			= k*(Pi.q*Pj.q)*displ/dist^3
end

# ╔═╡ f53fda47-82d3-4092-81a1-fce2452cd7b7
begin
	# Reset tick-count and time t
	tick		= 0;
	t 			= 0;
